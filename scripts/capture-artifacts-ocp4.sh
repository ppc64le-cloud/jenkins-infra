...
if [ ! -z "${BASTION_IP}" ]; then
    ...
    if [ $? -eq 0 ] ; then
        ...
        scp -i id_rsa -o StrictHostKeyChecking=no root@${BASTION_IP}:~/ui-screenshots.tar.gz .
        ...
        scp -i id_rsa -o StrictHostKeyChecking=no root@${BASTION_IP}:/tmp/nmstate/nmstate-e2e-*.txt .
