...
node_role=${APPLY_NODE_ROLES:="master worker"}
log_path=${LOG_PATH:="/var/crash"}
output_path="/root/kdump"
#===========================================
...
function gather_kdump_logs_from_node {
  ...
  until [[ -n "${debug_pod}" ]]; do
    ...
    debug_pod=$(oc get pods --namespace="default" 2>/dev/null | grep "$(echo $1 | tr -d .)-debug" | cut -d' ' -f1 || true)
    sleep 2
  done
...
function gather_kdump_logs {
  node_label="node-role.kubernetes.io/${1}"
