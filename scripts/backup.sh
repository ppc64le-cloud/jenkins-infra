...
done < <(ibmcloud cos objects --bucket "${JENKINS_BUCKET}" |  cut -d ' ' -f 1  | tail -n +6 |head -n -5)
...
done < <(ibmcloud cos objects --bucket "${INFLUXDB_BUCKET}" |  cut -d ' ' -f 1  | tail -n +6 |head -n -5)
